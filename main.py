...
from google.genai.types import HttpOptions  # ⬅️ NUEVO IMPORT
...
def process_clip(base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    clip_name = f"clip_{clip_index+1}"
    ...
    str(vertical_mp4_path), "ai-podcast-clipper-3", output_s3_key
)
...
class AiPodcastClipper:
    ...
    def load_model(self):
        ...
        self.gemini_client = genai.Client(
            ...
            http_options=HttpOptions(api_version="v1"),  # ⬅️ SOLO CAMBIO LÓGICO
        )
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client("s3")
        s3_client.download_file("ai-podcast-clipper-3", s3_key, str(video_path))
